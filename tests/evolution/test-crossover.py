...
class CrossTest(unittest.TestCase):
    ...
    def test_calls_cross_function_on_each_pair_of_matching_genes(self):
        """cross calls the appropriate crossover_function on pairs of genes from the given genomes"""
        genome_params = {
            "gene1": {"crossover_function": Mock()},
            "gene2": {"crossover_function": Mock()},
        }

        cross(self.genomes, genome_params)
    ...
    def test_returns_the_crossed_genome(self):
        """cross returns a new genome composed of genes returned by the crossover_functions"""
        genome_params = {
            "gene1": {"crossover_function": Mock(return_value=[10, 12, 14, 16])},
            "gene2": {"crossover_function": Mock(return_value=[True, True])},
        }

        crossed = cross(self.genomes, genome_params)
    ...
    def test_handles_a_single_parent(self):
        """if the given list of parents is a singleton, cross does not break"""
    ...
    def test_handles_more_than_two_parents(self):
        """if the given list of parents contains more than two elements, cross does not break"""
        genomes = [
            {"gene1": [1, 2, 3, 4], "gene2": [True, False]},
